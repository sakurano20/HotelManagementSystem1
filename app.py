...
UPLOAD_DIRS = [
    'static/uploads/rooms',
    'static/uploads/foods',
    'static/uploads'
]
...
def get_db():
    ...
    try:
        ...
        print(f"[DB] Connection error: {e}, reconnecting...")
        ca_path = os.path.join(os.path.dirname(__file__), "ca.pem")
        ssl_config = {"ca": ca_path} if os.path.exists(ca_path) else False
        ...
        print(f"[DB] Connecting to {conn_params['host']}:{conn_params['port']} as {conn_params['user']}")
...
def reservations():
    ...
    return render_template('reservations.html', reservations=reservations_list, available_rooms=available_rooms,
                         rooms=rooms, active_checkins=active_checkins, users=users)
...
def add_reservation():
    try:
        ...
        print(f"[ADD_RES] guest={guest_name}, email={email}, room_id={room_id}, check_in={check_in}, check_out={check_out}")
        ...
        try:
            ...
            if downpayment_amount < 0:
                flash('Downpayment cannot be negative', 'danger')
                return redirect(url_for('reservations'))
        except ValueError:
            flash('Invalid downpayment amount', 'danger')
            return redirect(url_for('reservations'))
        ...
        if check_in_date > max_date or check_out_date > max_date:
            flash(f'Reervations cannot exceed 5 months from today.', 'danger')
            return redirect(url_for('reservations'))
        ...
        if room:
            ...
            if downpayment_amount > total_price:
                cursor.close()
                flash('Downpayment cannot exceed total price', 'danger')
                return redirect(url_for('reservations'))
        cursor.close()
        ...
        if active_overlap:
            flash('Room is currently occupied.', 'danger')
            return redirect(url_for('reservations'))
        ...
        """, (guest_name, email, phone, room_id, check_in, check_out, downpayment_amount, request.form.get('fb_account', ''), request.form.get('agent_name', '')))
        db.commit()
        cursor.close()

        print(f"[ADD_RES] Success")
        flash('Reservation added successfully!', 'success')
        return redirect(url_for('reservations'))
    except Exception as e:
        print(f"[ADD_RES] ERROR: {e}")
        import traceback
        traceback.print_exc()
...
def add_checkin():
    ...
    if reservation_id:
        cursor.execute("""
            SELECT id, room_id, check_in, check_out, status, agent_name
    ...
    # Get agent_name from reservation if available
    agent_name = None
    ...
    cursor.execute("""
        ...
        VALUES (%s, %s, %s, %s, 'checked-in', %s, %s, %s, %s, %s)
...
def add_food():
    try:
        ...
        print(f"[ADD_FOOD] name={name}, price={price}, category={category}")
        ...
        if 'image' in request.files and request.files['image'].filename:
            ...
            try:
                os.makedirs(os.path.dirname(upload_path), exist_ok=True)
                image_file.save(upload_path)
                print(f"[ADD_FOOD] Image saved to {upload_path}")
            except Exception as e:
                print(f"[ADD_FOOD] Failed to save image: {e}")
                image_filename = None
    ...
    if request.method == 'POST':
        ...
        if request.form.get('setting_group') == 'general':
            ...
            general_settings = [
                ...
                ('enable_fb_account', request.form.get('enable_fb_account', 'false')),
                ('enable_agent_name', request.form.get('enable_agent_name', 'false')),
...
def reports():
    ...
    cursor.execute("""
        SELECT p.*, c.guest_name, rom.room_number, rom.type, c.agent_name
        FROM payments p
        JOIN checkins c ON p.checkin_id = c.id
        JOIN rooms rom ON c.room_id = rom.id
...
def migrate_add_users_role_column():
    ...
    try:
        ...
        if not result:
            ...
            # Column exists but might be enum - change to VARCHAR to allow 'agent'
            cursor.execute("ALTER TABLE users MODIFY COLUMN role VARCHAR(50) DEFAULT 'staff'")
        db.commit()
        cursor.close()
        return "Users role column updated successfully"
    except Exception as e:
        cursor.close()
        return f"Error: {str(e)}"
