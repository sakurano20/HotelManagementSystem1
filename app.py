...
@require_login
def migrate_add_fb_column():
    cursor = db.cursor()
    try:
        # Add to customers table
        try:
            cursor.execute("ALTER TABLE customers ADD COLUMN fb_account VARCHAR(255) DEFAULT NULL")
            db.commit()
        except Exception as e:
            print(f"Customers table update: {e}")

        # Add to reservations table
        try:
            cursor.execute("ALTER TABLE reservations ADD COLUMN fb_account VARCHAR(255) DEFAULT NULL")
            db.commit()
        except Exception as e:
            print(f"Reservations table update: {e}")

        cursor.close()
        return "FB account columns added successfully"
    except Exception as e:
        cursor.close()
